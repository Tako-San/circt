...
// hw.verilogName ("_y_output" in the canonical case).
...
hw.module @WithBody(in %a : i8, in %en : i1, out q : i8) {
  ...
  dbg.rootblock {
    dbg.decl_stmt "a"
    dbg.subblock guard "en" {
      dbg.connect_stmt "q" = "a" bp { enableRef = "en" }
    }
    dbg.subblock guard "en" negated true {
      dbg.connect_stmt "q" = "a"
    }
  }
  hw.output %a : i8
}
...
// connect inside the when-branch carries bp.enableRef resolved from
// the capture-when guard stack -- name tokens mapped to stable_ids.
// CHECK:           "bp":
...
hw.module @Dup(in %p : i1 loc(#locDup), out o : i1) {
  ...
  dbg.rootblock {
    dbg.connect_stmt "tmp" = "tmp" loc(#locDup)
  } loc(#locDup)
  hw.output %p : i1
} loc(#locDup)
