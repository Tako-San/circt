...
static bool isUhdiStampable(Operation *op) {
...
static StringRef kindPrefix(Operation *op) {
...
/// Excludes walk position so an unrelated dbg.* op insert/remove upstream
/// does not perturb existing siblings' prefixes; the per-prefix counter
/// handles same-bucket ordering.
static std::string hashFingerprint(Operation *op, StringRef moduleName) {
  std::string out;
...
static std::string hashPrefix(StringRef fp) {
  llvm::SHA256 h;
...
struct CounterTable {
  ...
  /// Bump the counter for `prefix` past `seenSuffix` (parsed as hex) so a
  /// fresh `next()` cannot collide with an already-stamped id of the form
  /// `<kind>_<prefix>_<seenSuffix>`. Without this, a second pass-run that
  /// inherits IR from a first run starts every counter at 0 and a new op
  /// hashing to an existing prefix re-issues an in-use id.
...
void UhdiInitPass::runOnOperation() {
  ...
  // Seed pass: every existing stable_id of shape `<kind>_<prefix>_<suffix>`
  // bumps `counters[prefix]` past its hex suffix. A second invocation
  // (firtool wires one before ExpandWhens and another after Inliner) thus
  // can't re-issue an in-use id when a freshly-inlined op hashes onto a
  // prefix that the first run already populated.
